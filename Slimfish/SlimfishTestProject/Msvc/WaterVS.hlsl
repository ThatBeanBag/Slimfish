...
VS_OUT main(VS_IN vIn)
{
	...
	/*float derivative = frequency * amplitude * cos(dot(direction, vIn.position) * frequency + gElapsedTime * phase);
	...
	float3 tangent = normalize(float3(0, 1, derivativeY));
	float3 normal = normalize(float3(-derivativeX, -derivativeY, 1));*/
