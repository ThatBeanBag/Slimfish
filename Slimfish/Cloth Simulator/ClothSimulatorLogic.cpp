...
bool CClothSimulatorLogic::Initialise()
{
	...
	m_Light.SetDirection(CVector3::Normalise(CVector3(0.8f, -1.0f, 0.8f)));
...
void CClothSimulatorLogic::Update(float deltaTime)
{
	...
	if (m_pGrabbedMass) {
		...
		m_pGrabbedMass->ApplyForce((intersectionPoint - m_pGrabbedMass->GetPosition()) * 100.0f);
	}
...
void CClothSimulatorLogic::CreateCloth(int width, int height, float restingDistance, float stiffness, float breakingForce)
{
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			CVector3 position(restingDistance * x, height - (y * restingDistance), 0.0f);

			auto pPointMass = std::make_unique<CPointMass>(position, 1.0f, 0.05f);
