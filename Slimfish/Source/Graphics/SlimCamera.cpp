...
namespace Slim {
	...
	const CRay CCamera::ScreenPointToRay(const CPoint& point) const
	{
		...
		if (CFrustum::GetProjectionMode() == EProjectionMode::ORTHOGRAPHIC) {
			...
			rayOrigin.SetX(inverseViewMatrix[0][3]);
			rayOrigin.SetY(inverseViewMatrix[1][3]);
			rayOrigin.SetZ(inverseViewMatrix[2][3]);

			/*auto viewMatrix = inverseViewMatrix.GetTranspose();
			viewMatrix.SetPosition(CVector3::s_ZERO);
			rayDirection.SetX(CVector3::DotProduct(viewMousePos, viewMatrix.GetRight()));
			rayDirection.SetY(CVector3::DotProduct(viewMousePos, viewMatrix.GetUp()));
			rayDirection.SetZ(CVector3::DotProduct(viewMousePos, viewMatrix.GetDirection()));
