...
//
// File Name	: SlimD3D11Conversions.cpp
// Description	: CSlimD3D11Conversions implementation file.
...
void D3D11Conversions::GetColour(const CColourValue& colour, float* pColourOut)
{
	pColourOut[0] = colour.m_r;
	pColourOut[1] = colour.m_g;
	pColourOut[2] = colour.m_b;
	pColourOut[3] = colour.m_a;
}

CColourValue D3D11Conversions::GetColour(const float* pColour)
{
	return CColourValue(pColour[0], pColour[1], pColour[2], pColour[3]);
}

	/*
CVector3 D3D11Conversions::GetVector3(const D3DXVECTOR3& vector)
{
	return CVector3(vector.x, vector.y, vector.z);
}

D3DXVECTOR3 D3D11Conversions::GetVector3(const CVector3& vector)
{
	return D3DXVECTOR3(vector.GetX(), vector.GetY(), vector.GetZ());
}

CMatrix4x4 D3D11Conversions::GetMatrix4x4(const D3DXMATRIX& d3dMatrix)
{
	CMatrix4x4 matrix;
...
D3DXMATRIX D3D11Conversions::GetMatrix4x4(const CMatrix4x4& matrix)
{
...
float[4] D3D11Conversions::GetColour(const CColourValue& colour)
{
	return D3DXCOLOR(colour.m_r, colour.m_g, colour.m_b, colour.m_a);
}


CColourValue D3D11Conversions::GetColour(const D3DXCOLOR& colour)
{
	return CColourValue(colour.a, colour.r, colour.g, colour.b);
}
*/

D3D11_USAGE D3D11Conversions::GetUsage(EGpuBufferUsage usage)
{
	if (usage == EGpuBufferUsage::DYNAMIC || 
		usage == EGpuBufferUsage::WRITE_ONLY) {
...
UINT D3D11Conversions::GetCPUAccessFlags(ETextureUsage usage)
{
	UINT d3dCpuAccess = 0;
	//Access
	...
	if (usage == ETextureUsage::READ_ONLY) {
		d3dCpuAccess |= D3D11_CPU_ACCESS_READ;
	}

	return d3dCpuAccess;
}

D3D11_USAGE D3D11Conversions::GetUsage(ETextureUsage usage)
{
	if (usage == ETextureUsage::DYNAMIC ||
...
D3D11_MAP D3D11Conversions::GetLockType(EGpuBufferLockType lockType)
{
	switch (lockType) {
...
DXGI_FORMAT D3D11Conversions::GetFormat(CInputElement::EFormat inputElementFormat)
{
...
DXGI_FORMAT D3D11Conversions::GetPixelFormat(ETexturePixelFormat format)
{
	switch (format) {
...
ETexturePixelFormat D3D11Conversions::GetPixelFormat(DXGI_FORMAT format)
{
	switch (format) {
...
D3D11_PRIMITIVE_TOPOLOGY D3D11Conversions::GetPrimitiveType(EPrimitiveType primitiveType)
{
...
DXGI_FORMAT D3D11Conversions::GetFormat(AIndexGpuBuffer::EIndexType indexType)
{
	switch (indexType) {
...
D3D11_TEXTURE_ADDRESS_MODE D3D11Conversions::GetAddressMode(ETextureAddressMode addressMode)
{
	switch (addressMode) {
...
D3D11_FILTER D3D11Conversions::GetFilter(ETextureFilterType minFilter, ETextureFilterType magFilter, ETextureFilterType mipFilter)
{
...
EShaderConstantType D3D11Conversions::GetShaderConstantType(D3D11_SHADER_TYPE_DESC variableTypeDesc)
{
...
D3D11_BLEND_OP D3D11Conversions::GetBlendOperation(EBlendOperation blendOperation)
{
...
D3D11_BLEND D3D11Conversions::GetBlendFactor(EBlendFactor blendFactor)
{
	switch (blendFactor) {
...
D3D11_CULL_MODE D3D11Conversions::GetCullingMode(ECullingMode cullingMode)
{
	switch (cullingMode) {
...
D3D11_FILL_MODE D3D11Conversions::GetFillMode(EFillMode fillMode)
{
	switch (fillMode){
...
D3D11_COMPARISON_FUNC D3D11Conversions::GetComparisonFunction(EComparisonFunction function)
{
	switch (function) {
...
D3D11_STENCIL_OP D3D11Conversions::GetStencilOperation(EStencilOperation operation)
{
	switch (operation) {
